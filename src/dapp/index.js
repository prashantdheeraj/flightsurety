...
(async () => {
  ...
  let contract = new Contract('localhost', () => {
    ...
    contract.isAuthorizedCaller((error,result) => {
      display('Contract Owner Authorization Status', 'Check if contract is Authorized', [{label: 'Authorizarion Status', error: error, value: result }])
    });
    ...
    //Contract owner authorises a user
    DOM.elid('authorise-user').addEventListener('click', async () => {
      // destructure
      let input = DOM.elid('auth-user').value
      // Write transaction
      const { result, error } = await contract.authoriseUser(input)
      console.log(result);
      console.log(error);      
    })
    ...
    DOM.elid('submit-oracle').addEventListener('click', async () => {
      ...
      console.log(landing)
      // Write transaction
      const key = await contract.fetchFlightStatus(flight, destination, landing);
      console.log(key)
    })
    ...
    DOM.elid('register-flight').addEventListener('click', async () => {
      const startTime = new Date(DOM.elid('regFlightTakeOff').value).getTime()
      const landTime = new Date(DOM.elid('regFlightLanding').value).getTime()
      const flightCode = DOM.elid('regFlightRef').value
      const ticketCost = DOM.elid('regFlightPrice').value
      const origin = DOM.elid('regFlightFrom').value
      const destination = DOM.elid('regFlightTo').value
      console.log(landTime);
      console.log(destination);
      const { flightIdentifier, error } =  await contract.registerFlight(
        flightCode,
        origin,
        destination,
        startTime,
      ...
      console.log("Flight identifier is :" + flightIdentifier);
      console.log(error);
    ...
    DOM.elid('buy').addEventListener('click', async () => {
      ...
      fetch('http://localhost:3000/flights')
        ...
        .then(async flight => {
          ...
          const { flight: { flightCode, desitination, landTime, ticketCost } } = flight[0]
          // execute transaction
          ...
          display(
            ...
            'Book flight',
            [{
              label: `${flightCode} to ${desitination} lands at ${landTime}`,
              error: error,
